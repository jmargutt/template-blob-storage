...
def get_blob_client(blob_path, container):
...
def upload_file(container, file_path='messages.csv', blob_path='messages.csv'):
    ...
    blob_client = get_blob_client(blob_path, container)
...
def list_files(container):
    ...
    container_client = BlobServiceClient.from_connection_string(os.getenv("CONNECTION")).get_container_client(container)
...
def main():
    ...
    print(list_files(container))
    download_file(container, 'test', 'ids/usage/jor-deh-staging-logs-extractor-s')
    upload_file(container, 'test', 'ids/usage/jor-deh-staging-logs-extractor-s')
